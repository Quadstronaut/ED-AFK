...
def calibrate_menu(
    *,
    ...
    if not is_owner:
        ...
        print_fn(f"how many presses to reach 'CmdrFour'.")
        group_dir = _prompt_direction(
            input_fn, print_fn, "Arrow direction to reach CmdrFour in the list",
            default="down",
        )
        group_count = _prompt_nonneg_int(
            input_fn, print_fn, f"Press {group_dir.upper()} N times to reach CmdrFour; N ="
        )
