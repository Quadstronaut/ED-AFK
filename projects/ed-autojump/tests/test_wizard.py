...
def test_setup_creds_skips_commander_with_existing_cred(tmp_path, monkeypatch):
    ...
    _write_cred(cred_dir, "account1")  # CmdrOne already authed
    cfg = LauncherConfig()
    mel = _FakeMel()
    # Inputs are not consumed because no commander needs login.
    r = setup_frontier_creds(
        ["CmdrOne"], launcher_cfg=cfg, mel=mel,
        input_fn=_make_inputs([]), print_fn=lambda _: None,
    )
    assert "CmdrOne" in r.skipped
    assert "CmdrOne" not in r.succeeded
...
def test_setup_creds_launches_mel_for_missing_cred(tmp_path, monkeypatch):
    ...
    r = setup_frontier_creds(
        ["CmdrTwo"], launcher_cfg=cfg, mel=mel,
...
def test_setup_creds_marks_failed_when_cred_not_written(tmp_path, monkeypatch):
    ...
    r = setup_frontier_creds(
        ["CmdrThree"], launcher_cfg=cfg, mel=mel,
        input_fn=_make_inputs([""]),
        print_fn=lambda _: None,
        sleep=lambda _: None,
    )
    assert "CmdrThree" in r.failed
    assert "CmdrThree" not in r.succeeded
...
def test_setup_creds_handles_multiple_commanders(tmp_path, monkeypatch):
    ...
    cred_dir = _setup_cred_env(tmp_path, monkeypatch)
    _write_cred(cred_dir, "account1")  # CmdrOne present
    # CmdrTwo: succeeds (mel writes cred). CmdrThree: fails (no write).
    written_for = set()
    def writer(slug):
        if slug == "account2":
    ...
    assert r.skipped == ["CmdrOne"]
    assert r.succeeded == ["CmdrTwo"]
    assert r.failed == ["CmdrThree"]
...
def test_calibrate_menu_member_captures_direction_and_count():
    ...
    Down×3 to CmdrFour (vertical group list)."""
    r = calibrate_menu(
        commander="CmdrOne", is_owner=False,
...
def test_calibrate_menu_owner_skips_group_questions():
    ...
    r = calibrate_menu(
        commander="CmdrFour", is_owner=True,
