...
def _loadgame(group: Optional[str] = "CmdrFour", commander: str = "CmdrOne",
              game_mode: str = "Group"):
    payload = {
...
def test_wait_for_load_game_returns_when_loadgame_fires():
    ...
    assert r.found is True
    assert r.event.commander == "CmdrOne"
...
def test_wait_for_load_game_verifies_group_when_required():
    ...
    with pytest.raises(LoadGameMismatch, match="WrongGroup"):
        wait_for_load_game(
            tail, timeout_s=10.0, expected_group="CmdrFour",
            poll_interval_s=0.1, clock=clock.now, sleep=clock.sleep,
        )


def test_wait_for_load_game_verifies_commander_when_required():
    tail = FakeTail([[_loadgame(commander="CmdrTwo")]])
    clock = FakeClock()
    with pytest.raises(LoadGameMismatch, match="CmdrTwo"):
        wait_for_load_game(
            tail, timeout_s=10.0, expected_commander="CmdrOne",
            poll_interval_s=0.1, clock=clock.now, sleep=clock.sleep,
        )
