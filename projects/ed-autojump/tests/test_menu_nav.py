...
- Member commander (e.g. CmdrOne): full flow including select-group
- Owner commander (e.g. CmdrFour): skips select-group step
...
def _cfg(calibration: dict):
    return MenuNavConfig(
        ...
        group_owner_commander="CmdrFour",
        calibration=calibration,
    )


def test_member_commander_sends_full_sequence():
    """CmdrOne: Continue → 1×Down → Enter (PG) → 2×Down → Enter (Quad) → Enter (Launch)."""
    cfg = _cfg({"CmdrOne": _calibration_for_member()})
    sender = _FakeSender()
    sleeps: list[float] = []
    nav = MenuNavigator(sender=sender, config=cfg, sleep=sleeps.append)
    nav.navigate(commander="CmdrOne")
...
def test_owner_commander_skips_select_group_step():
    """CmdrFour: Continue → 1×Down → Enter (PG) → Enter (Launch) — 4 keys, not 7."""
    cfg = _cfg({"CmdrFour": _calibration_for_owner()})
    sender = _FakeSender()
    nav = MenuNavigator(sender=sender, config=cfg, sleep=lambda _: None)
    nav.navigate(commander="CmdrFour")
    sc = _recorded_press_scancodes(sender)
    assert sc == [
        SPACE_SCANCODE,
        DOWN_ARROW_SCANCODE,
...
def test_horizontal_pg_nav_uses_right_arrow():
    ...
    cfg = _cfg({"CmdrOne": _calibration_horizontal_pg()})
    sender = _FakeSender()
    nav = MenuNavigator(sender=sender, config=cfg, sleep=lambda _: None)
    nav.navigate(commander="CmdrOne")
    sc = _recorded_press_scancodes(sender)
...
def test_disabled_navigator_refuses_to_run():
    ...
    cfg = MenuNavConfig(enabled=False, calibration={"CmdrOne": _calibration_for_member()})
    sender = _FakeSender()
    nav = MenuNavigator(sender=sender, config=cfg, sleep=lambda _: None)
...
def test_navigator_respects_key_delay_ms():
    ...
    cfg = MenuNavConfig(
        ...
        dismiss_dialogs=False,
        group_owner_commander="CmdrFour",
        calibration={"CmdrFour": _calibration_for_owner()},
    )
    sender = _FakeSender()
    sleeps: list[float] = []
    nav = MenuNavigator(sender=sender, config=cfg, sleep=sleeps.append)
