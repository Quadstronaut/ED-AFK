...
| `engage_supercruise` | `poll_s: float = 0.8`, `max_charge_s: float = 60.0` | If already in SC, no-op. Else press `Supercruise` and gate on game signals only: `SupercruiseEntry` event or Supercruise flag = success; FsdCharging true→false without entry = failure. No success-window clock; `max_charge_s` is the operator-sanctioned stuck-state watchdog. |
| `hold_alignment` | `until_event: str = "StartJump"`, `poll_s: float = 0.8`, `align_tol`, `gain`, `min_press`, `max_press`, `samples`, `max_charge_s: float = 60.0` | **The post-engage_jump gate.** Micro-corrects compass alignment during the FSD spool; exits on `until_event` (or its state flag: FsdJump / Supercruise bit) = success, FsdCharging true→false without it = failure, FsdCooldown before any charge = refused, operator abort, or `max_charge_s` (60s stuck-state watchdog — operator-sanctioned 2026-06-06: "nothing should take a minute to jump"; far above any real ~15-20s spool, never gates a healthy charge). |

### Journal / timing gates

`wait_for_event` and `wait_cooldown` are **deleted** — a timeout/duration as a success/failure gate is banned. Use `hold_alignment` (event/state-gated) after `engage_jump` / for SC entry, and `wait_cooldown_clear` (flag-gated) for the smack cooldown.

| action | params | what it does |
|---|---|---|
| `wait_cooldown_clear` | `poll_s: float = 0.5` | Block until the **FsdCooldown status flag** clears. Already clear → instant pass. Fails closed without status. |
