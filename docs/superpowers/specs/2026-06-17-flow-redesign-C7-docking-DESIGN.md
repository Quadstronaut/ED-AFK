...
== 5. <7.5km OCR ==
navpanel_reader._DISTANCE_RE matches LS|LY only (line 81); distance column explicitly DROPPED as redundant (40-43). No km/Mm parser, no <7.5km comparator in repo. NEW parse_station_distance_km(text)->float|None (km/Mm regex, Mm*1000, None on unread) + NEW step_dock_close_to_range (bounded backstop, fail-closed on unread, throttle-zero finally ram-guard via resolve_nav_region per-ship crop #19). **UPDATE 2026-06-21 (`C7-DOCKING-DISTANCE-FINDING.md`, frames CAPTURED):** read the `<7.5km` distance from the RIGHT-SIDE cockpit target panel (it persists on the Contacts tab where REQUEST DOCKING is pressed); the nav-LIST distance VANISHES on the Contacts tab, so do NOT source the gate from it. Per-ship crop (#19). Fixtures: `navpanel_nav_station_km_1080.png` + `navpanel_contacts_request_docking_1080.png`.
